...
const vcard = 'BEGIN:VCARD\n' // jangan ganti
            ...
            + 'FN:BrayyLomli\n' // ganti aja 
            + 'ORG: BrayyIsGblk;\n' // ganti aja
            + 'TEL;type=CELL;type=VOICE;waid=62895618626337:+62895618626337\n'  //ganti nomor lu
...
healawal = 50 // biarin
potionawal = 1 // biarin
replitx = '*SUBREK Brayy Bot*' 
ari = '*YT Brayy Bot*'
limitawal = 10 // terserah mau ganti apa gak 
memberlimit = 1 // terserah mau ganti apa gak 
namabot = 'Brayy Bot'
cr = '*「 Brayy Bot 」*' //ganti nama bot

// Ottoriasasi
tf = '├➤' // Biarin Bree / ubah aja tapi yg bener
rf = '══─❲' // Biarin Bree / ubah aja tapi yg bener
fer = '❳─═' // Biarin Bree / ubah aja tapi yg bener
...
/******** OWNER NUMBER**********/
const ownerNumber = ["62895618626337@s.whatsapp.net"]   //ganti nomor lu
//================================ Batas ========================
battre = batre
charg = chargi
charging = false
...
// ANTIVIRTEX BUATAN FERNAZER
		        ...
		        if (isGroupAdmins) return reply('ATMIN BEBAS BOSS🗿🤙')
		        client.updatePresence(from, Presence.composing)
		        var kic = `${sender.split("@")[0]}@s.whatsapp.net`
...
${tf} Nomor Bot : wa.me/+15145872835
╰══──────────══─⊱
...
${tf} NOTE : SC INI DI SUSUN FERNAZER SENDIRI !!
...
break
		...
		case 'request':
					...
					client.sendMessage('62895618626337@s.whatsapp.net', options, text, {quoted: mek})
					client.sendMessage('62895618626337@s.whatsapp.net', options, text, {quoted: mek})
					client.sendMessage('62895628626337@s.whatsapp.net', options, text, {quoted: mek})
...
║╠ Contoh: 
║╠ #suratto 6285725486864|Hai Juga
║╚▸
╚═══ ◤ *${namabot}* ◢ `
				...
				case 'public':
          	    ...
          	    uptime = process.uptime()
          	    anu  = {contextInfo:{'stanzaId': "B826873620DD5947E683E3ABE663F263", 'participant': `${ownerNumber}@s.whatsapp.net`, 'remoteJid': '6283136505591-1614953337@g.us', 'quotedMessage': {"imageMessage": {"caption": `「 𝐑𝐔𝐍𝐓𝐈𝐌𝐄 」\n${kyun(uptime)}`, 'jpegThumbnail': fs.readFileSync('./lib/logo.jpeg')}}}}
          	    banChats = false
		    ...
		    client.sendMessage(from, fs.readFileSync(`./sticker/${sender}.webp`), MessageType.sticker, {quoted: mek})
	           ...
	           if (!isGroup) return reply(ind.group)
				...
				case 'potion':                     
				    ...
				    client.sendMessage(from, hasil, image, { quoted: mek, caption: `◪ *「 DEATH 」\n\n├─ ❏ *Tempat  ${ad}*\n├─ ❏ *MONEY : Rp.${money}*\n├─ ❏ *EXP : ${adven}Xp*\n└─ ❏ Potion : ${poti}` })
				    await limitAdd(sender)
                ...
                if (!isRegistered) return reply(ind.noregis())
                if (isBanned) return reply('Maaf kamu sudah terbenned !')
                if (!isNsfw) return reply(`MODE NSFW TIDAK AKTIF\nSILAHKAN KETIK ${prefix}nsfw(HANYA OWNER NGAB)`)
			    if (isLimit(sender)) return 
		        await limitAdd(sender)
               ...
               case 'art':
                ...
                case 'megumin':
                case 'wallnime':
                if (!isNsfw) return reply('*Maaf anda nsfw tidak diaktifkan')
                if (!isRegistered) return reply( ind.noregis())
                if (!isLimit(sender)) return reply(ind.limitend(pushname))
